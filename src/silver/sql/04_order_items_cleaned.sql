CREATE TABLE IF NOT EXISTS {order_items_cleaned} AS
SELECT
  order_id,
  product_id,
...
MERGE WITH SCHEMA EVOLUTION INTO {order_items_cleaned} AS target
USING (
  SELECT
    order_id,
    product_id,
...
COMMENT ON TABLE {order_items_cleaned} IS 'Deduplicated, type-cast order line items; soft-deleted rows kept and flagged via _is_deleted/_deleted_at. Grain: 1 row / order line item.';
