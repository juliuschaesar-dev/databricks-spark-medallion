CREATE TABLE IF NOT EXISTS {products_cleaned} AS
SELECT
  product_id,
...
MERGE WITH SCHEMA EVOLUTION INTO {products_cleaned} AS target
USING (
  SELECT
    product_id,
    TRIM(product_name)             AS product_name,
...
COMMENT ON TABLE {products_cleaned} IS 'Deduplicated, type-cast products; soft-deleted rows kept and flagged via _is_deleted/_deleted_at. Grain: 1 row / product.';
